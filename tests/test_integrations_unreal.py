...
def test_unreal_is_a_bundled_addin():
    ...
    assert integrations.bundled_addin_version("unreal") == "0.2.17"


def test_unreal_object_mode_uses_selected_roots_and_editor_transaction():
    source = (Path(__file__).parents[1] / "trackball_daemon/plugins/unreal/TrackballNav" /
              "Content/Python/trackball_nav.py").read_text(encoding="utf-8")
...
def test_install_copies_plugin_and_enables(isolated_config, tmp_path, monkeypatch):
    ...
    assert integrations.installed_addin_version("unreal") == "0.2.17"
