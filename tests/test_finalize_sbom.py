...
def test_finalize_sbom_adds_dynamic_version_reproducibly(tmp_path):
    ...
    _write_sbom(sbom, {
        ...
        "name": "astrolabe-daemon",
        "type": "application",
    })

    finalize_sbom(sbom, name="astrolabe-daemon", version="0.1.75")
    first = sbom.read_bytes()
    finalize_sbom(sbom, name="astrolabe-daemon", version="0.1.75")

    assert sbom.read_bytes() == first
    root = json.loads(first)["metadata"]["component"]
    assert root["name"] == "astrolabe-daemon"
...
def test_finalize_sbom_rejects_wrong_or_missing_root(tmp_path):
    ...
    with pytest.raises(ValueError, match="does not match"):
        finalize_sbom(sbom, name="astrolabe-daemon", version="1")

    _write_sbom(sbom, {"name": "astrolabe-daemon", "type": "library"})
