"""Offline unit test for the AutoCAD plugin's NavMath "pointer" pivot path.
...
behaviour, to_pointer parallel zoom keeps the point's screen fraction fixed, perspective falls
...
def test_navmath_pointer_pivot_invariants():
    res = subprocess.run(
        ["dotnet", "run", "-c", "Release", "--project", _PROJ],
        capture_output=True, text=True, timeout=300,
    )
    assert res.returncode == 0, (
        "NavMath pointer-pivot tests failed:\n" + res.stdout + res.stderr)
