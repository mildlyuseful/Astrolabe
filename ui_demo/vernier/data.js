...
const APPS = [
  ...
  app("godot", "Godot", {
    tier: "experimental", rich: true, noRoll: true, process: "godot.exe (or godot_v4*.exe)",
    pivots: PIVOTS_CAMERA, orbitStyles: ["turntable"], twistActions: ["zoom", "dolly", "none"],
    zoomBehaviors: ["zoom", "dolly"], features: ["zoom_behavior"],
    exclude: ["lock_horizon", "level_horizon"],
    integration: {
      detected: "detected v4.6: C:\\Program Files\\Godot\\Godot_v4.6-stable_win64.exe",
      compat: "supported", verified: "Godot 4.4 through 4.7",
      installModel: "Godot EditorPlugin copied and enabled per project.",
      setup: "Set up finds running/recent projects, copies addons/trackball_nav, and enables res://addons/trackball_nav/plugin.cfg.",
      manual: "Copy trackball_daemon\\plugins\\godot\\trackball_nav to <YourProject>\\addons\\trackball_nav, then enable Trackball Nav under Project > Project Settings > Plugins. A staged copy is also placed under %APPDATA%\\Mildly Useful\\Astrolabe\\godot when no project is found.",
      health: "Reload the project, focus a 3D editor viewport, and look for connected. Check %APPDATA%\\Mildly Useful\\Astrolabe\\godot_addin.log on failure.",
      security: "Copies unsigned GDScript into each detected project and edits that project's project.godot to enable the plugin. No elevation or machine-wide setting change.",
      confirmation: "Godot setup copies editor scripts into every detected project and edits project.godot to enable Trackball Nav. Review or use the manual project-local steps if automatic project edits are unwanted.",
      setupRequired: true, action: "Set up", installed: false, enabled: false,
      runtime: { state: "disabled", detail: "integration disabled" },
    },
  }),
...
const SYSTEM_DEFAULTS = {
  ...
  apps: {
    ...
    godot: {
      ...
      "navigation.orbit.twist_action": "zoom",
      "navigation.zoom.behavior": "dolly",
    },
